    ...
    def run(self):
        ...
        while not job_queue.empty():
            try:
                ...
                exeprint = exe[:]
                for i in xrange(len(exeprint)):
                    if exeprint[i][0] == '/':
                        exeprint[i] = os.path.relpath(exeprint[i])

                print("[" + self.name + "] Now running: " + " ".join(exeprint) + " " + now)
                ...
                print("[" + self.name + "] Done. Took " + total_time + " seconds. (" + exeprint[0] + " ===> " + os.path.basename(self.statistics_filename) + ")")
...
def makeExeName(buildfolder, experiment, method):
    path = buildfolder + "/src/" + experiment + "_" + method + "_main"
...
parser.add_argument('-p', '--parameter', nargs="*",
                    help="Which parameters to match")
parser.add_argument('-t', '--test', action='store_true',
                    help="Just print which runs would have been made")
args = parser.parse_args()

# Preprocess args matching argument
args.parameter = [] if args.parameter is None else [tuple(unicode(p).split("=")) for p in args.parameter]
...
with open(args.experiments_file) as data_file:
    try:
        ...
        print("Could not load json from file", args.experiments_file)
        print(e)
        sys.exit(0)
...
for e in data["experiments"]:
    if args.experiment is not None and e["name"] not in args.experiment:
        continue
    efolder = e["folder"] if "folder" in e else e["name"]
    buildfolder = e["buildfolder"] if "buildfolder" in e else "../build"

    for m in e["algorithms"]:
        if args.algorithm is not None and m["name"] not in args.algorithm:
            continue
        if "skip" in m and m["skip"] is True:
            continue

        exe = makeExeName(buildfolder, e["name"], m["name"])
        if not os.path.isfile(exe):
            print(exe, "not found!")
            sys.exit(1)

        mfolder = m["folder"] if "folder" in m else m["name"]

        output_folder = folder + "/" + efolder + "/" + mfolder
        ...
        for global_parameter in e["global_parameters"]:
            for config in m["configs"]:
                ...
                for parameter_list in parameter_lists:
                    ...
                    # Check that we match all things
                    pml = [(p[0], unicode(p[1])) for p in parameter_list]
                    if not all(x in pml for x in args.parameter):
                        continue
